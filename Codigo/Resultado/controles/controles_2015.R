...
# microdados de 2015 pra escolas 

df <- read_csv("TS_ESCOLA_2015.csv")
...
df <- df %>% 
  ...
  select(c(1:6, 9, 34, 60))


# controles alunos

# microdados 2015 pra alunos

base <- read_csv("TS_ALUNO_9EF_2015.csv") %>% 
  filter(ID_UF == 23,
         ID_DEPENDENCIA_ADM == 3,
...
controles_15 <- left_join(df, escolas)

controles_15 <- controles_15 %>% 
  select(-c(1:3, 9))


write_csv(controles_15, "controles_15.csv")
